...
using QnSCommunityCount.AspMvc.Models.Modules.Language;
using QnSCommunityCount.Contracts.Modules.Language;
using QnSCommunityCount.Contracts.Persistence.Account;
using AccountManger = QnSCommunityCount.Adapters.Modules.Account.AccountManager;

namespace QnSCommunityCount.AspMvc.Modules.Language
{
    public partial class Translator
    {
        ...
        private static string CreatePredicate() => $"AppName.Equals(\"{nameof(QnSCommunityCount)}\") && {nameof(KeyLanguage)} == {(int)KeyLanguage} && {nameof(ValueLanguage)} == {(int)ValueLanguage}";
            ...
            if (handled == false)
            {
                ...
                try
                {
                    ...
                    foreach (var item in keyValuePairs.Where(i => i.Key.HasContent()))
                    {
                        ...
                        if (entity != null)
                        {
                            entity.AppName = nameof(QnSCommunityCount);
