...
namespace QnSCommunityCount.Logic.Modules.Security
{
    internal partial class JsonWebToken
    {
        static JsonWebToken()
        {
            ...
            Issuer = Configuration.Settings.Get($"JwtSetting:{nameof(Issuer)}", nameof(QnSCommunityCount));
            Audience = Configuration.Settings.Get($"JwtSetting:{nameof(Audience)}", nameof(Logic));
            var settingValue = Configuration.Settings.Get($"JwtSetting:{nameof(TimeOutInSec)}", Authorization.TimeOutInSec.ToString());
